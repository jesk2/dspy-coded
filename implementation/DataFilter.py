...
print(dataset)  # Print dataset to understand its structure

data_train = [{'instruction': '', 'response': ex['text']} for ex in dataset['train'].select(range(100))]
...
evaluator = Evaluate(devset=data_train, metric=custom_metric, num_threads=1, display_progress=True)
results = evaluator()
...
class DataFilter(dspy.Module):
    ...
    def filter(self):
        scores = [score for _, _, _, score in results[1]]  # Extract scores from results
